...
public class Main2 extends JFrame {
	...
	final int SCRWIDTH = (int) java.awt.Toolkit.getDefaultToolkit().getScreenSize().getWidth();
	final int SCRHEIGHT = (int) java.awt.Toolkit.getDefaultToolkit().getScreenSize().getHeight();
	...
	public static void main(String[] args) throws URISyntaxException {
		...
		String uriString2 = Main2.class.getResource("nyan.mp3").toURI().toString();
	...
	public Main2() {
		...
		}
		{
			...
			for (int i = 0; i < helpLabels.length; i++) {
				...
				helpLabels[i].setBounds(SCRWIDTH - (w + marg), marg + i * h, w, h);
		...
		xp = new JProgressBar(0, 100);
		{
			...
			int h = 40;
			int w = 75;
			xp.setBounds(2 * marg + w, SCRHEIGHT - (h + marg), SCRWIDTH - (w + 3 * marg), h);
		...
		addKeyListener(new KeyAdapter() {
			...
			public void keyPressed(KeyEvent arg0) {
				if (arg0.getKeyCode() == KeyEvent.VK_C) {
					nyandown++;
					Random r = new Random();
					contentPane.setBackground(new Color(Constants.MINCOLOR + r.nextInt(256 - Constants.MINCOLOR),
							Constants.MINCOLOR + r.nextInt(256 - Constants.MINCOLOR),
							Constants.MINCOLOR + r.nextInt(256 - Constants.MINCOLOR)));
					updateHelp();
	...
	private static String getColor() {
		return contentPane.getBackground().getRed() + "," + contentPane.getBackground().getBlue() + ","
				+ contentPane.getBackground().getGreen();
	}

	private static void updateHelp() {
		String[] tab = table(3,
				new String[][] { new String[] { "[SPACE]", "hold and release to start" },
						new String[] { "[N]", "generate new Scramble" }, new String[] { "[P]", "Switch Puzzle (wip)" },
						new String[] { "[C]", "to alternate Color (current: " + getColor() + ")" } });
	...
	public static void updateStrings() {
		String[] lines0 = tableToLines(3, new String[][] {
				new String[] { "Ao5", toTimestamp(SortUtils.aoN(entries, 5)), "  Best Ao5:",
						toTimestamp(SortUtils.bestAoN(entries, 5)), "  Mean:", toTimestamp(SortUtils.mean(entries)),
						"  Best:", toTimestamp(SortUtils.best(entries)) },
				new String[] { "Ao12", toTimestamp(SortUtils.aoN(entries, 12)), "  Best Ao12:",
						toTimestamp(SortUtils.bestAoN(entries, 12)), "  Median:",
						toTimestamp(SortUtils.median(entries)), "  Worst:", toTimestamp(SortUtils.worst(entries)) },
				new String[] { "Ao100", toTimestamp(SortUtils.aoN(entries, 100)), "  Best Ao100:",
						toTimestamp(SortUtils.bestAoN(entries, 100)), "  Deviation:",
	...
	public static void updateXp() {
		...
		xpLabel.setText(Progress.getXp() + "/" + Progress.xpAt(Progress.getLevel()) + " exp.");
		...
		for (int j = 0; j < lines.length; j++) {
			...
			for (int i = 0; i < s.length; i++) {
				if (s[i].length() < max[i] + min) {
					s[i] += times(max[i] + ((i != (s.length - 1)) ? min : 0) - s[i].length(), ' ');
